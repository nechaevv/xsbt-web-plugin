...
object WebappPlugin extends Plugin {
  ...
  def webappSettings0(classpathConfig: Configuration):Seq[Setting[_]] = warSettings0(classpathConfig) ++ Seq(
    ...
    env := None,
    webXml := None,
    deployment <<= (webappResources, fullClasspath in classpathConfig, scanDirectories, scanInterval, env, webXml) map {
      (rs, cp, sd, si, env, webXml) =>
	Deployment(rs, cp.map(_.data), sd, si, env, webXml)
    }
  )
